...
class TerminalApplication(GOApplication):
    ...
    def initialize(self):
        ...
        plugin_list = set()
        for plugin in list(
            list(self.plugins['py'].keys()) +
            list(self.plugins['js'].keys()) +
            list(self.plugins['css'].keys())):
            if '.' in plugin:
        ...
        for name, plugin in list(self.plugins['py'].items()):
            try:
        ...
        for plugin_name, hooks in list(self.plugin_hooks.items()):
            plugin_name = plugin_name.split('.')[-1]
            if 'WebSocket' in hooks:
                # Apply the plugin's WebSocket actions
                for ws_command, func in list(hooks['WebSocket'].items()):
    ...
    def new_terminal(self, settings):
        ...
        *settings* as the parameters.  If a terminal already exists with the
        ...
        for mode, setting in list(m.term.expanded_modes.items()):
    ...
    def move_terminal(self, settings):
        ...
        *SESSIONS* dict.
        ...
        if new_location not in self.ws.locations:
            ...
            existing_terms = [
                a for a in list(self.ws.locations[
    ...
    def get_locations(self):
        ...
        """
        term_locations = {}
        for location, obj in list(self.ws.locations.items()):
            terms = obj.get('terminal', None)
            if terms:
                term_locations[location] = list(terms.keys())
...
#   * A way to view multiple shared terminals on a single page with the option to break them out into individual windows/tabs.
    ...
    def notify_permissions(self):
        ...
        for user in users:
            ...
            if not upn:
                continue
            for share_id, share_dict in list(shared_terms.items()):
                try:
    ...
    def attach_shared_terminal(self, settings):
        ...
        *settings['share_id']* if the user is authorized to view the share or if
        ...
        for mode, setting in list(multiplex.term.expanded_modes.items()):
            self.mode_handler(term, mode, setting)
...
def init(settings):
    ...
    for name, command in list(term_settings['commands'].items()):
    ...
    # Attach plugin hooks
    plugin_hooks = {}
    for name, plugin in list(plugins['py'].items()):
        try:
