...
SEPARATOR = "\U000f0f0f" # The character used to separate frames in the log
...
def escape_escape_seq(text, preserve_renditions=True, rstrip=True):
    ...
    for char in text:
        if not esc_buffer:
            if char == '\x1b': # Start of an ESC sequence
                esc_buffer = char
            ...
            esc_buffer += char
            if char == '\x07' or esc_buffer.endswith('\x1b\\'): # Likely title
                esc_buffer = '' # Nobody wants to see your naked ESC sequence
                continue
            ...
            if match_obj:
                ...
                esc_buffer = '' # Just when you thought you've ESC'd...
                continue
            ...
            if match_obj:
                ...
                if csi_type == 'm' and preserve_renditions: # mmmmmm!
                    ...
                    out += ' ' * spaces # Add an equivalent amount of spaces
                esc_buffer = '' # Make room for more!
                continue
    if rstrip:
...
def flatten_log(log_path, file_like, preserve_renditions=True, show_esc=False):
    ...
    for count, frame in enumerate(get_frames(log_path)):
        ...
        if show_esc:
            frame_time = frame_time.strftime('\x1b[0m%b %d %H:%M:%S')
        else: # Renditions preserved == I want pretty.  Make the date bold:
            frame_time = frame_time.strftime('\x1b[0;1m%b %d %H:%M:%S\x1b[m')
        if not show_esc:
            term.write(frame[14:])
        if term.capture:
        ...
        for char in frame[14:]:
            ...
            if char == '\n':
                ...
                if cr:
                    out_line += "^M"
                    file_like.write((frame_time + ' ').encode('utf-8'))
                    if show_esc:
                        adjusted = raw(out_line)
                    else:
                        adjusted = escape_escape_seq(out_line, rstrip=True)
...
def get_256_colors(container="gateone"):
    ...
    for i in range(256):
        ...
        fg_rev =(
            ...
            "inherit;}" % (container, i, color_map[i]))
        bg_rev =(
