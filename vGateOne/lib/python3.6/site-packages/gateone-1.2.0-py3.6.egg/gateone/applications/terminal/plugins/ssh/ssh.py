...
def open_sub_channel(self, term):
    ...
    m.writeline('echo -e "\\033]0;Term %s sub-channel\\007"' % term)
    return m
...
def wait_for_prompt(term, cmd, errorback, callback, m_instance, matched):
    ...
    m_instance.writeline((
        'echo -e "{rs}"; ' # Echo the first READY_STRING
        '{cmd}; ' # Execute the command in question
        'echo -e "{rs}"' # READY_STRING again so we can capture the between
