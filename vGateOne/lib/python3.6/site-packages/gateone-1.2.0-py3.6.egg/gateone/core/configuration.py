...
def print_help(commands):
    ...
    by_group = {}
    for option in list(options._options.values()):
    ...
    for line in lines:
        print(("%s %s" % (' ', line)))
    print("")
    for module, command_dict in list(commands.items()):
...
def generate_server_conf(installed=True):
    ...
    """
    logger.info(_(
        "Gate One settings are incomplete.  A new <settings_dir>/10server.conf"
        " will be generated."))
...
def combine_css(path, container, settings_dir=None):
    ...
    *container* as the replacement for templates use of '#{{container}}'.
            ...
            if os.path.isdir(theme_dir):
                ...
                for filename in filelist:
                    ...
                    if filename.endswith('.css'):
                        ...
                        theme_writers[theme].write(
                            "\n/* ------ filepath: %s ------ */\n" % filepath)
                        theme_writers[theme].write(resource(filepath) + '\n')
                        ...
                        for filename in filelist:
                            ...
                            if filename.endswith('.css'):
                                ...
                                theme_writers[theme].write(
                                    resource(filepath) + '\n')
        f.flush()
    for writer in list(theme_writers.values()):
        writer.flush()
        writer.close()
    ...
    for theme in list(theme_writers.keys()):
        combined_theme_path = "%s_theme_%s" % (
            path.split('.css')[0], theme)
