...
class Pattern(object):
    ...
    def __init__(self, pattern, callback,
        ...
        if isinstance(callback, str):
            ...
            self.callback = lambda m, match: m.write(str(callback))
        else:
...
class BaseMultiplex(object):
    ...
    def term_write(self, stream):
        ...
        if self.CALLBACK_UPDATE in self.callbacks:
            for callback in list(self.callbacks[self.CALLBACK_UPDATE].values()):
    ...
    def dump_html(self, full=False, client_id='0'):
        ...
        if client_id not in self.prev_output:
            self.prev_output[client_id] = [None for a in range(self.rows-1)]
        try:
            scrollback, html = ([], [])
            if self.term:
                try:
            ...
            if html:
                if not full:
                    count = 0
                    for line1, line2 in zip(self.prev_output[client_id], html):
                        if line1 != line2:
...
class MultiplexPOSIXIOLoop(BaseMultiplex):
    ...
    def terminate(self):
        ...
        for callback in list(self.callbacks[self.CALLBACK_EXIT].values()):
            self._call_callback(callback)
        ...
        with ProcessPoolExecutor(max_workers=1) as pool:
            ...
            for callback in list(finalize_callbacks.values()):
