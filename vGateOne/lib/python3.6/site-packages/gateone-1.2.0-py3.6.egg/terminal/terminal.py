...
# I've been staring at obscure symbols far too much lately ⨀_⨀
        128: '€', # Euro sign
        129: ' ', # Unknown (Using non-breaking spaces for all unknowns)
        ...
        157: 'Ø', # Upper-case slashed zero--using same as empty set (216)
        ...
        216: 'Ø', # Latin capital letter O with slash (aka "empty set")
    ...
    if isinstance(e, (UnicodeEncodeError, UnicodeTranslateError)):
        s = ['%s' % specials[c] for c in chars[e.start:e.end]]
        return ''.join(s), e.end
    else:
        s = ['%s' % specials[c] for c in chars[e.start:e.end]]
        return ''.join(s), e.end
...
def spanify_screen(state_obj):
    ...
    for linecount, line_rendition in enumerate(zip(screen, renditions)):
        ...
        charcount = 0
        for char, rend in zip(line, rendition):
            ...
            if rend == prev_rendition:
                ...
                prev_rendition = rend
            if changed and rend:
                classes = map(rendition_classes.get, rend)
        ...
        if outline:
            ...
            for whatever in range(spancount):
                outline += "</span>"
    ...
    for whatever in range(spancount): # Bit of cleanup to be safe
        results[-1] += "</span>"
    ...
    def capture(self, data, term_instance):
        ...
        if term_instance.em_dimensions:
            ...
            if height <= term_instance.em_dimensions['height']:
                ...
                term_instance.screen[img_Y][img_X] = ' ' # Empty old location
    ...
    def capture(self, data, term_instance):
        ...
        *term_instance* can be used by overrides of this function to make
        ...
        if self.thumbnail:
            ...
            img_X = term_instance.cursorX
            ref = term_instance.screen[img_Y][img_X]
            term_instance.screen[img_Y][img_X] = ' ' # No longer at this loc
            if term_instance.cursorY < 8: # Icons are about ~8 newlines high
                for line in range(8 - term_instance.cursorY):
                    term_instance.newline()
...
class Terminal(object):
    ...
    class_prefix = '✈' # Prefix used with HTML output span class names
    ...
    charsets = {
        ...
        '0': { # Line drawing mode
            ...
            109: '└',
            110: '┼',
            111: '⎺', # All these bars and not a drink!
            112: '⎻',
            113: '─',
            114: '⎼',
            115: '⎽',
            ...
            124: '≠',
            125: '£',
            126: '·' # Centered dot--who comes up with this stuff?!?
        }
    }
    ...
    def initialize(self, rows=24, cols=80, em_dimensions=None):
        ...
        self.cursor_home = 0
        self.cur_rendition = chr(1000) # Should always be reset ([0])
        self.init_screen()
        ...
        self.renditions_store = {
            ' ': [], # Nada, nothing, no rendition.  Not the same as below
    ...
    def init_screen(self):
        ...
        self.screen = [array('u', ' ' * self.cols) for a in range(self.rows)]
        # Tabstops
    ...
    def init_renditions(self, rendition=chr(1000)): # Match unicode_counter
        """
        ...
        self.renditions = [
            array('u', rendition * self.cols) for a in range(self.rows)]
    ...
    def remove_all_callbacks(self, identifier):
        ...
        """
        for event, identifiers in list(self.callbacks.items()):
            try:
                del self.callbacks[event][identifier]
            except KeyError:
    ...
    def send_message(self, message):
        ...
        try:
            for callback in list(self.callbacks[CALLBACK_MESSAGE].values()):
                callback(message)
        except TypeError:
            pass
    ...
    def send_update(self):
        ...
        try:
            for callback in list(self.callbacks[CALLBACK_CHANGED].values()):
                callback()
        except TypeError:
            pass
    ...
    def send_cursor_update(self):
        ...
        try:
            for callback in list(self.callbacks[CALLBACK_CURSOR_POS].values()):
                callback()
        except TypeError:
            pass
    ...
    def reset(self, *args, **kwargs):
        ...
        try:
            for callback in list(self.callbacks[CALLBACK_RESET].values()):
                callback()
        except TypeError:
            pass
        ...
        *self.em_dimensions* (which is currently only used by image output).
        ...
        if rows < self.rows: # Remove rows from the top
            ...
            for i in range(rows - self.rows):
                line = array('u', ' ' * self.cols)
                renditions = array('u', chr(1000) * self.cols)
        ...
        if cols > self.cols: # Add cols to the right
            for i in range(self.rows):
                for j in range(cols - self.cols):
                    ...
                    self.renditions[i].append(chr(1000))
        self.cols = cols
    ...
    def set_title(self, title):
        ...
        self.title = title
        try:
            for callback in list(self.callbacks[CALLBACK_TITLE].values()):
                callback()
...
# rendition line to come up with a logical rendition for that exact spot.
    ...
    def _dsr_get_cursor_position(self):
        ...
        try:
            for callback in list(self.callbacks[CALLBACK_DSR].values()):
    ...
    def _set_line_params(self, param):
        ...
        if param == 8:
            ...
            self.init_renditions()
            self.screen = [
                array('u', 'E' * self.cols) for a in range(self.rows)]
    ...
    def abort_capture(self):
        ...
        self.send_update()
        self.send_message(_('File capture aborted.'))
    ...
    def scroll_up(self, n=1):
        ...
        empty_line = array('u', ' ' * self.cols) # Line full of spaces
        empty_rend = array('u', chr(1000) * self.cols)
        ...
        try:
            for callback in list(self.callbacks[CALLBACK_SCROLL_UP].values()):
                callback()
        except TypeError:
            pass
    ...
    def scroll_down(self, n=1):
        ...
        for x in range(int(n)):
            ...
            empty_line = array('u', chr(1000) * self.cols)
    ...
    def bell(self):
        ...
        if not self.esc_buffer: # We're not in the middle of an esc sequence
            ...
            try:
                for callback in list(self.callbacks[CALLBACK_BELL].values()):
                    callback()
            except TypeError:
                pass
    ...
    def set_expanded_mode(self, setting):
        ...
        if setting.startswith('?'):
            ...
            try:
                for callback in list(self.callbacks[CALLBACK_MODE].values()):
    ...
    def insert_characters(self, n=1):
        ...
        for i in range(n):
            ...
            self.screen[self.cursorY].insert(self.cursorX, ' ')
    ...
    def delete_characters(self, n=1):
        ...
        for i in range(n):
            try:
                ...
                self.screen[self.cursorY].append(' ')
                self.renditions[self.cursorY].pop(self.cursorX)
                self.renditions[self.cursorY].append(chr(1000))
            except IndexError:
    ...
    def _erase_characters(self, n=1):
        ...
        n = min(n, distance)
        for i in range(n):
            self.screen[self.cursorY][self.cursorX+i] = ' '
            self.renditions[self.cursorY][self.cursorX+i] = chr(1000)
    ...
    def cursor_left(self, n=1):
        ...
        try:
            ...
            char = ' ' # This is a safe default/fallback
    ...
    def clear_screen_from_cursor_down(self):
        ...
        self.screen[self.cursorY+1:] = [
            array('u', ' ' * self.cols) for a in self.screen[self.cursorY+1:]
        ]
    ...
    def clear_screen_from_cursor_up(self):
        ...
        self.screen[:self.cursorY+1] = [
            array('u', ' ' * self.cols) for a in self.screen[:self.cursorY]
        ]
    ...
    def clear_line_from_cursor_right(self):
        ...
        spaces = array('u', ' '*len(self.screen[self.cursorY][self.cursorX:]))
        renditions = array('u',
    ...
    def clear_line(self):
        ...
        self.screen[self.cursorY] = array('u', ' ' * self.cols)
        c = self.cur_rendition
    ...
    def set_led_state(self, n):
        ...
        for led in leds:
            ...
            if led == 0:
                ...
                self.leds[led] = True
        try:
            for callback in list(self.callbacks[CALLBACK_LEDS].values()):
                callback(led)
        except TypeError:
            pass
    ...
    def _set_rendition(self, n):
        ...
        *n* is expected to be a string of ECMA-48 rendition numbers separated by
        ...
        if not n: # or \x1b[m (reset)
            ...
            self.cur_rendition = chr(1000) # Should be reset (e.g. [0])
        ...
        if out_renditions[0] == 0:
            ...
            if reduced not in list(self.renditions_store.values()):
                ...
                for k, v in list(self.renditions_store.items()):
                    if reduced == v:
    ...
    def _opt_handler(self, chars):
        ...
        """
        try:
            for callback in list(self.callbacks[CALLBACK_OPT].values()):
                callback(chars)
        except TypeError:
...
# (mostly) but the performance was TERRIBLE.  Still needs investigation...
    ...
    def _spanify_scrollback(self):
        ...
        html_entities = {"&": "&amp;", '<': '&lt;', '>': '&gt;'}
        cursor_span = '<span class="%scursor">' % self.class_prefix
        for line, rendition in zip(screen, renditions):
            combined = (line + rendition).tounicode()
            if has_cache and combined in html_cache:
        ...
        for line in self.screen:
            ...
            for char in line:
                if len(char) > 1: # This is an image (or similar)
                    line_out += '⬚' # Use a dotted square as a placeholder
                else:
                    line_out += char
            out.append(line_out)
        self.modified = False
